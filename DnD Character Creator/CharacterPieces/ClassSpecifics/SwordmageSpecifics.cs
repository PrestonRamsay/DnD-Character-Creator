    ...
    public static class SwordmageSpecifics
    {
        ...
        public static CharacterClass Features(CharacterClass result)
        {
            ...
            if (lvl >= 2)
            {
                ...
                result.ClassFeatures.Add("Enchanted Blade", "bonus to teleport to you, +1D6 dmg, 1/LR - change the dmg type to: " +
                    "\nCold, Force, Lightning, Necrotic, Radiant or Fire");
            }
            if (lvl >= 3)
            {
                ...
                var archetype = new List<string> { "Assault Style", "Ensnaring Style", "Shiedling Style" };
                int answer = CLIHelper.PrintChoices(msg, archetype);

                if (answer == 0)
                {
                    ArcaneSwordStyle = archetype[0];
